...
import {
  getAssetMintAccountLabelInfo,
  getAssetSolAccountLabel,
  getAssetTokenAccountLabelInfo,
} from '@utils/tokens';
import React, { useEffect } from 'react';
import { getProgramName } from '@components/instructions/programs/names';
import { AssetAccount } from '@utils/uiTypes/assets';

const AssetAccountSelect = ({
  onChange,
  value,
  error,
  assetAccounts = [],
    ...
    if (value) {
      ...
      if (value.isSol || value.isToken) {
        return getTokenAccountLabelComponent(
          value.isSol
            ? getAssetSolAccountLabel(value)
            : getAssetTokenAccountLabelInfo(value),
        );
      } else {
        switch (accountType) {
          ...
          case GovernanceAccountType.MintGovernanceV2:
            return getMintAccountLabelComponent(
              getAssetMintAccountLabelInfo(value),
            );
  ...
  useEffect(() => {
    if (assetAccounts.length == 1 && autoselectFirst) {
      //wait for microtask queue to be empty
      setTimeout(() => {
        onChange(assetAccounts[0]);
      });
    }
...
export default AssetAccountSelect;
